...
class TokenSimulator:
    def __init__(self, master):
        self.master = master
        self.master.title("Muse Token Selling Simulator")

        # Initial values
        self.initial_tokens = tk.IntVar(value=735)
        self.initial_valuation = 18.67
        ...
        self.scenario_percentage_label.grid(row=0, column=3, padx=5, pady=5, sticky="e")
        self.scenario_percentage_entry = tk.Entry(input_frame, textvariable=self.sell_percentage, width=5)
        self.scenario_percentage_entry.grid(row=0, column=4, padx=5, pady=5, sticky="w")

        self.scenario_price_label = tk.Label(input_frame, text="Price:")
        self.scenario_price_label.grid(row=0, column=5, padx=5, pady=5, sticky="e")
        self.scenario_price_entry = tk.Entry(input_frame, textvariable=self.sell_valuation, width=5)
        self.scenario_price_entry.grid(row=0, column=6, padx=5, pady=5, sticky="w")

        self.max_calculations_label = tk.Label(input_frame, text="Max Calculations:")
        self.max_calculations_label.grid(row=0, column=7, padx=5, pady=5, sticky="e")
        self.max_calculations_entry = tk.Entry(input_frame, textvariable=self.max_calculations, width=5)
        self.max_calculations_entry.grid(row=0, column=8, padx=5, pady=5, sticky="w")

        self.gap_label = tk.Label(input_frame, text="Sell Gap:")
        self.gap_label.grid(row=0, column=9, padx=5, pady=5, sticky="e")
        self.gap_entry = tk.Entry(input_frame, textvariable=self.gap, width=5)
        self.gap_entry.grid(row=0, column=10, padx=5, pady=5, sticky="w")

        # Buttons
        self.run_simulation_button = tk.Button(master, text="Run Simulation", width=15, command=self.run_simulation)
        self.run_simulation_button.grid(row=0, column=3, pady=5, padx=5, sticky="w")  # Change sticky to "w"

        self.export_button = tk.Button(master, text="Export to CSV", width=15, command=self.export_to_csv)
        self.export_button.grid(row=0, column=4, pady=5, padx=5, sticky="w")  # Adjust column to 3
    ...
    def run_simulation(self):
        ...
        current_iteration = 1
        current_valuation = float(self.sell_valuation.get())  # Update this line
        ...
        for _ in range(max_calculations):
            ...
            sold_percentage = self.sell_percentage.get() / 100
            sell_valuation = self.sell_valuation.get()
            ...
            simulation_id = f"{current_tokens}_{self.sell_percentage.get()}@{sell_valuation}"

            # Update the results tree
            self.results_tree.insert("", tk.END, text=str(current_iteration),
                                     values=(simulation_id, f"${current_valuation:.2f}", f"{self.sell_percentage.get()}%",
            ...
            current_iteration += 1
            current_valuation += self.gap.get()
            current_tokens = int(remaining_tokens)

            # Append the simulation result to the list
            self.simulation_results.append((simulation_id, f"${current_valuation:.2f}", f"{self.sell_percentage.get()}%",
                                            f"${sold_value:.2f}", f"{int(sold_tokens)}", f"{int(remaining_tokens)}",
                                            f"${accumulative_value:.2f}", f"${remaining_value:.2f}",
                                            f"${accumulative_value + remaining_value:.2f}"))
